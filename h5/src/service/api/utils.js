  ...
  return function () {
    try {
      ...
      if (Object.prototype.toString.apply(e) === '[object Error]') {
        if (e.type == 'AppServiceSdkKnownError') throw e
...
function getRealRoute (pathname, url) {
  ...
  if ((n && (url = addHTMLSuffix(url)), url.indexOf('/') === 0)) { return url.substr(1) }
  if (url.indexOf('./') === 0) return getRealRoute(pathname, url.substr(2), !1)
  var index,
    urlArrLength,
    urlArr = url.split('/')
  for (
...
function assign () {
  // endext 对象合并
  for (
    var argLeng = arguments.length, args = Array(argLeng), n = 0;
    n < argLeng;
    n++
  ) {
    args[n] = arguments[n]
...
function addHTMLSuffix (url) {
  // 给url加上。html的扩展名
  if (typeof url !== 'string') throw new A('wx.redirectTo: invalid url:' + url)
  var urlArr = url.split('?')
  urlArr[0] += '.html'
...
function arrayBufferToBase64 (buffer) {
  for (
    var res = '', arr = new Uint8Array(buffer), arrLeng = arr.byteLength, r = 0;
    r < arrLeng;
    r++
  ) {
...
var Components = {
  ...
  text: {
    '1.0.0': [],
    '1.1.0': ['selectable'],
    '1.4.0': [{ space: ['ensp', 'emsp', 'nbsp'] }, 'decode']
  },
  textarea: {
    '1.0.0': [
      'value',
...
var APIs = {
  ...
  showToast: {
    '1.0.0': [{ object: ['title', 'icon', 'duration', 'mask'] }],
    '1.1.0': [{ object: ['image'] }]
  },
  ...
  showActionSheet: {
    '1.0.0': [{ object: ['itemList', 'itemColor'] }, { success: ['tapIndex'] }]
  },
  ...
  getBluetoothAdapterState: {
    '1.1.0': [{ success: ['discovering', 'available'] }]
  },
  onBluetoothAdapterStateChange: {
    '1.1.0': [{ callback: ['available', 'discovering'] }]
  },
  ...
  onBLEConnectionStateChange: {
    '1.1.1': [{ callback: ['deviceId', 'connected'] }]
  },
  ...
  getStorageInfo: {
    '1.0.0': [{ success: ['keys', 'currentSize', 'limitSize'] }]
  },
  ...
  saveFile: {
    '1.0.0': [{ object: ['tempFilePath'] }, { success: ['savedFilePath'] }]
  },
  getSavedFileList: { '1.0.0': [{ success: ['fileList'] }] },
  getSavedFileInfo: {
    '1.0.0': [{ object: ['filePath'] }, { success: ['size', 'createTime'] }]
  },
  removeSavedFile: { '1.0.0': [{ object: ['filePath'] }] },
  openDocument: {
    '1.0.0': [{ object: ['filePath'] }],
    '1.4.0': [{ object: ['fileType'] }]
  },
  getBackgroundAudioManager: { '1.2.0': [] },
  getFileInfo: {
    '1.4.0': [
      { object: ['filePath', { digestAlgorithm: ['md5', 'sha1'] }] },
      { success: ['size', 'digest'] }
    ]
  },
  ...
  onBeaconServiceChange: {
    '1.2.0': [{ callback: ['available', 'discovering'] }]
  },
  getLocation: {
    '1.0.0': [
      { object: ['type'] },
  ...
  openLocation: {
    '1.0.0': [{ object: ['latitude', 'longitude', 'scale', 'name', 'address'] }]
  },
  ...
  playBackgroundAudio: {
    '1.0.0': [{ object: ['dataUrl', 'title', 'coverImgUrl'] }]
  },
  ...
  getImageInfo: {
    '1.0.0': [{ object: ['src'] }, { success: ['width', 'height', 'path'] }]
  },
  ...
  navigateToMiniProgram: {
    '1.3.0': [{ object: ['appId', 'path', 'extraData', 'envVersion'] }]
  },
  uploadFile: {
    '1.0.0': [
  ...
  hideShareMenu: { '1.1.0': [] },
  updateShareMenu: {
    '1.2.0': [{ object: ['withShareTicket'] }],
    '1.4.0': [{ object: ['dynamic', 'widget'] }]
  },
  getShareInfo: {
    '1.1.0': [
      { object: ['shareTicket'] },
  ...
  onBLEConnectionStateChanged: {
    '1.1.0': [{ callback: ['deviceId', 'connected'] }]
  },
  ...
  if (!attribute) {
    ...
    for (var key in component) {
      for (var i = 0; i < component[key].length; i++) {
        if (
          typeof component[key][i] === 'string' &&
          component[key][i] == attribute
        ) {
          return true
    ...
    if (!method) {
      ...
      for (var key in api) {
        for (var i = 0; i < key.length; i++) {
          if (typeof api[key][i] === 'object' && api[key][i][method]) {
            if (!param) {
              return true
            } else {
              for (var j = 0; j < api[key][i][method].length; j++) {
                if (
                  typeof api[key][i][method][j] === 'string' &&
                  api[key][i][method][j] == param
                ) {
                  return true
                } else if (
                  typeof api[key][i][method][j] === 'object' &&
                  api[key][i][method][j][param]
                ) {
                  if (!options) {
                    return true
                  } else if (
                    api[key][i][method][j][param].indexOf(options) > -1
                  ) {
                    return true
                  } else {
                    return false
...
export default {
  ...
  anyTypeToString: surroundByTryCatchFactory(
    anyTypeToString,
    'anyTypeToString'
  ),
  stringToAnyType: surroundByTryCatchFactory(
    stringToAnyType,
    'stringToAnyType'
  ),
